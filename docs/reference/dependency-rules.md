...
| `feature` | Business-domain packages: agent, broker, llm, tool, session, task, todo, subagent, cron, selflearn, command, skill, plugin, mcp, hook, setting, persona, search, inspector, reminder, reviewer, image, memory | Cohesive product capabilities with their own state and lifecycle. |
